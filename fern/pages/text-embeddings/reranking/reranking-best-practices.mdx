...
Our `rerank-v3.5` and `rerank-v3.0` models are trained with a context length of 4096 tokens. Both the input from the query and your documents are counted towards this limit, and the query can account for up to _half_ of this limit. On other words, if your query is larger than 2048 tokens, it will be truncated to the first 2048 tokens.
