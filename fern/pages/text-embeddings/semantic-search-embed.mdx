...
- `model`: Here we choose `embed-v4.0`, which generates embeddings of size 1536, but we will set the output dimension to 1024
...
The Embed endpoint also supports multilingual semantic search via `embed-v4.0` and then previous `embed-multilingual-...` models. This means you can perform semantic search on texts in different languages.
