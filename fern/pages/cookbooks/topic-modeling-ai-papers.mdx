...
To follow along with this tutorial, you need to be familiar with Python. Make sure you have python version 3.6+ installed. You can also use Google Colab to try out the project in the cloud. Finally, you need to have a Cohere Account.
...
### Get Topic Essences
Then, the `get_essence` function calls out to a Cohere Command model to create an 'essentialized' description of the papers in a given cluster. Like the 'short names' from above this will improve the readibility of our plot, because otherwise it would be of limited use.
...
Congratulations! You have created the word embeddings and visualized them using a scatter plot. First, you need to reduce the dimensions of the word embeddings. 
