...
---
Retrieval Augmented Generation (RAG) is a method for generating text using additional information fetched from an external data source, which can greatly increase the accuracy of the response. When used in conjunction with [Command A+](/docs/command-a-plus), [Command A](/docs/command-a), or [Command R7B](/docs/command-r7b), the [Chat API](/reference/chat) makes it easy to generate text that is grounded on supplementary information.
