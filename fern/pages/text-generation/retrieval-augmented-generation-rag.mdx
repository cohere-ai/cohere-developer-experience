...
The [Chat API](/reference/chat) can generate search queries using Tools. In the example below, we ask the model to suggest some search queries that would be useful when answering the question.
...
You can customize the preamble and/or the tool definition to generate queries that are more relevant to your use case.
