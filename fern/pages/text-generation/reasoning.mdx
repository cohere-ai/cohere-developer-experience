...
Reasoning models represent an advanced approach to AI that enables more sophisticated problem-solving capabilities. Cohere's _hybrid_ reasoning models can work by generating internal reasoning processes before delivering their final responses, allowing them to tackle complex tasks with deeper analysis.
...
for event in response:
    if event.type == "content-delta":
        ...
        if event.delta.message.content.type == "text":
...
A thinking token budget can also be specified, to set an upper limit on how many thinking tokens the model can produce. When the budget is exceeded, the model will immediately proceed with the final response.
