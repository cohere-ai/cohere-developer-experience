...
Reasoning models represent an advanced approach to AI that enables more sophisticated problem-solving capabilities. Cohere's reasoning models are _hybrid_, meaning reasoning can be enabled (in which case they generatie internal reasoning processes before delivering their final responses) or disabled (in which case they function the way any other LLM would). 
