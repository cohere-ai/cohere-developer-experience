...
A conversation starts with a system message, or a [system instruction](/docs/preambles), to help steer a chatbot’s response toward certain characteristics.
...
In the example below, the system instruction ('preamble') provides context for the assistant's task (task and context) and encourages the generation of rhymes as much as possible (style guide).
...
- [Documentation on system messages](/docs/preambles)

## Creating a single-turn conversation

Let's start with a single-turn conversation, which doesn't require the chatbot to maintain any conversation state. 

Here, we are also adding a custom system instruction for generating concise response, just to keep the outputs brief for this tutorial.
