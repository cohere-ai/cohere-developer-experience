...
With Cohere, you can generate text embeddings through the [Embed endpoint](/docs/cohere-embed), which supports over 100 languages.
...
- `model`: Here we choose `embed-v4.0`, which can generate embeddings of different sizes (we'll use 1024).
...
In the example below, we repeat the steps of performing semantic search with one difference – changing the model type to the multilingual version. Here, we use the `embed-v4.0` model to search a _French_ version of the FAQ list using an _English_ query.
