...
**visually**, when you inspect a vault's attestation details in the Model Vault app.

## Automatic verification (the client)

You do not have to do anything manually for verification to occur. The client-side **TNG (Trusted
Network Gateway)** component, whether the TNG client embedded in the SDK or a standalone TNG proxy,
runs the full check on every session and **refuses the connection** if anything is wrong:
...
The attestation also runs a set of detailed GPU verification checks. In the Model Vault app these appear as a
