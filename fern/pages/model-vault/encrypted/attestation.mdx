...
<img src="../../../assets/images/model-vault/model-vault-encrypted-passport-architecture.svg" />

### Components

- **Client**: runs **TNG (Trusted Network Gateway)**, a client-side confidential-computing component that handles OHTTP encryption/decryption and attestation verification. It comes in two forms: a **TNG client** embedded in the SDK, or a standalone **TNG proxy**. Either one embeds an **ITA verifier** that uses ITA's public keys to confirm attestation tokens were genuinely signed by ITA. The client never talks to a confidential VM directly; all traffic goes through the load balancer.
...
The client-side TNG connects through the load balancer, which routes the first request to one VM
...
OHTTP, and returns it. The client-side TNG decrypts the response. The load balancer never sees
