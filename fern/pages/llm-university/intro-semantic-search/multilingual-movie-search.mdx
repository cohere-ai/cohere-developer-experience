...
Use the `embed-multilingual-v3.0` model to embed the movie description. If the text is longer than 512 tokens,
the co.embed function (truncate parameter) will truncate it for you. The co.embed function will convert the
`query_text` into an embedding, which is a 1024-dimensional vector of floats that uniquely represents a movie
description.
...
vectors_to_search = np.array(co.embed(model=model_name, texts=[query_text], input_type="search_query",
