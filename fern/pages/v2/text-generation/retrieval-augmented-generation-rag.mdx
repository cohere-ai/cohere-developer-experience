...
<Info>
When streaming with the accurate citations option, there can be occasional (less than 1% of the time) differences between the text as it is streamed token-by-token and the citations. This can cause citation offsets to be incorrect. Learn more about this as well as the best practices in [this notebook](https://github.com/cohere-ai/cohere-developer-experience/blob/main/notebooks/guides/rag_streaming_citations_best_practice.ipynb).
</Info>
