...
Here is a chunked version of the document. (we don’t cover the chunking process here, but if you’d like to learn more, see this cookbook on [chunking strategies](https://github.com/cohere-ai/notebooks/blob/main/notebooks/guides/Chunking_strategies.ipynb).)
...
if response.message.citations:
    ...
    for citation in response.message.citations:
        print(f"Start: {citation.start} | End: {citation.end} | Text: '{citation.text}'", end ="")
        if citation.sources:
...
# After
co.chat(
  model="command-r-plus-08-2024", # or any model of your choice
  messages=[{"role": "user", "content": """Write a short summary from the following text in bullet point format,
  in different words.
