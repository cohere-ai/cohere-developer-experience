...
You're likely already familiar with streaming. When you ask the model a question using the Coral UI, the interface doesn't output a single block of text, instead it _streams_ the text out a few words at a time. In many user interfaces enabling streaming improves the user experience by lowering the perceived latency.
