...
You can generate text embeddings Embeddings API by passing a list of strings as the `input` parameter. You can also specify in `encoding_format` the type of embeddings to be generated such as `float`, `int8`, or others ([complete list](https://docs.cohere.com/reference/embed#request.body.embedding_types)).
...
print(response.data[0].embedding[:5]) # Display the first 5 dimensions
```
