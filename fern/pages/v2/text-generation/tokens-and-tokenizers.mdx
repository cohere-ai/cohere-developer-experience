...
hidden: false 
description: "This document explains that language models use tokens instead of characters or bytes, with common words having unique tokens and longer, less frequent words being encoded into multiple tokens. The number of tokens per word varies based on text complexity."
...
co.tokenize(text="caterpillar", model="command-r-08-2024") # -> [74, 2340,107771]
```
...
co.tokenize(text="caterpillar", model="command-r-08-2024", offline=False) # -> [74, 2340,107771], no tokenizer config was downloaded
```
