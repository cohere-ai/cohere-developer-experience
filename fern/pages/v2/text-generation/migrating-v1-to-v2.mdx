...
response = co_v2.chat(model=model, messages=messages, tools=web_search_tool)
...
while response.message.tool_calls:
    ...
    for idx, tc in enumerate(response.message.tool_calls):
        ...
        messages.append(
            {"role": "tool", "tool_call_id": tc.id, "tool_content": tool_content}
        )
...
res_v2 = co_v2.chat_stream(
    model="command-r-plus-08-2024", messages=messages, documents=documents_v2
)
...
res_v2 = co_v2.chat(model="command-r-plus-08-2024", messages=messages, tools=tools_v2)
...
if res_v2.message.tool_calls:
    for tc in res_v2.message.tool_calls:
        ...
        messages.append(
            {"role": "tool", "tool_call_id": tc.id, "tool_content": tool_content_v2}
        )
