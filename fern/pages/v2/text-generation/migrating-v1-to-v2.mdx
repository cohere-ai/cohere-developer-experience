...
- Message structure: 
   ...
   - v2: uses a single `messages` parameter consisting of a list of messages with `content` and a `role` (`system`, `user`, `assistant`, or `tool`). The a message with the `system` role in v2 replaces the `preamble` parameter from v1.
