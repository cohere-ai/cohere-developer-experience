...
Besides, this results in faster development. You don’t need to spend a lot of time prompt engineering the model to avoid hallucinations.

For example, here we create a tool that can retrieve sales data for a given day. It specifies that a function call must call the function `query_daily_sales_report` which contains a parameter called `day`. The Structured Outputs feature ensures that the tool call will always follow the tool definitions.
...
<Info title="Important"> 
Structured Outputs (Tools) does NOT support this scenario: when a tool is defined with one or more parameters but there is no parameter set as `required`. There must be at least one `required` parameter.
</Info>
