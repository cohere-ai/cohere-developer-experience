...
As a reminder, even if you specify a system message in your dataset, the default inference request to `co.chat()` will have an empty system message. If you want to make an inference request with system message, [please pass the parameter](/reference/chat) `preamble`. 
...
response = co.chat(
    ...
    # Required - user message. Optional (to specify a system message (preamble))
    messages=[
        {
            "role": "system",
...
response_2 = co.chat(
    model=get_response.finetuned_model.id + "-ft",
    # optional (to specify a system message (preamble))
    messages=[
        {
            "role": "system",
