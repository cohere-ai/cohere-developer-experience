...
Note that the code snippets below are in Python, but you can find the equivalent code for other languages ([if they're supported](https://docs.cohere.com/docs/cohere-works-everywhere)).

## Prerequisites

Here are the steps you'll need to get set up in advance of running Cohere models on Amazon Bedrock.

- Subscribe to Cohere's models on Amazon Bedrock. For more details, refer to the [Amazon documentation](https://docs.aws.amazon.com/bedrock/latest/userguide/model-access.html).
- You'll also have to configure your authentication credentials for AWS. See the [Amazon documentation](https://boto3.amazonaws.com/v1/documentation/api/latest/guide/quickstart.html#configuration) for more information.
...
Note that we've released [multimodal embeddings models](https://docs.cohere.com/docs/multimodal-embeddings) that are able to handle images in addition to text.
