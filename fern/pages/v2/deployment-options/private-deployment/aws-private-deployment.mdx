...
slug: v2/docs/aws-private-deployment
hidden: false
description: >-
...
export LICENSE_ID="<YOUR_LICENSE_ID>"export DOCKER_CONFIG=$(mktemp -d)cat <<EOF > "${DOCKER_CONFIG}/config.json" { "auths": { "proxy.replicated.com": {"auth": "$(echo -n "${LICENSE_ID}:${LICENSE_ID}" | base64 | tr -d '\n')"}}EOF
```
...
<Note>Please reach out to the [Cohere team](mailto:team@cohere.com) to get the latest model tag.</Note>
