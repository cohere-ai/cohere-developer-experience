...
Model Vault is billed as a Cohere-managed service rather than through your cloud provider account (for example, AWS), so cloud-provider credits typically do not apply.

Pricing depends on the models you select and each model’s performance requirements. Cohere manages the underlying infrastructure and scaling, and customers can choose between two pricing models:

**Fixed**
- **Commitment**: Monthly or annual
- **Capacity**: Each Vault model runs with a fixed number of instances (no autoscaling)
- **Sizing**: Instance counts are determined through a sizing exercise or a production trial (for example, based on expected load)

**Flex**
- **Commitment**: Monthly or annual
- **Baseline capacity**: Each model starts with a fixed minimum number of instances
- **Autoscaling**: Models can scale up or down based on request rate and agreed latency SLOs
- **Overages**: Additional capacity is billed per instance-hour
- **Max capacity**: Each model has a maximum instance cap it can scale up to
...
You may also want to compare Model Vault against the operational and capacity costs of running inference directly on your cloud provider.
