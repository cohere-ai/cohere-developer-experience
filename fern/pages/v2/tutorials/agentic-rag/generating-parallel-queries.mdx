...
But the second query is more complex as it requires breaking down the query into two separate queries, one for Apple and one for Google.
...
In this part, we'll learn how agentic RAG with Cohere handles parallel query execution.
- Query expansion
...
Additionally, because the question asks about 2 different things, retrieving information using the same query as the user's may not be the most optimal. Instead, the query needs to be expanded, or split into multiple parts, each retrieving its own set of documents.

Thus, the agent expands the original query into 2 queries.
