...
| `command-r-plus-04-2024` (deprecated 09/15/2025)   | Command R+ was an instruction-following conversational model that performs language tasks at a higher quality, more reliably, and with a longer context than previous models. It is best suited for complex RAG workflows and multi-step tool use.   | Text                 | 128k           | 4k                    | [Chat](/reference/chat)|
| `command-r-plus`         (deprecated 09/15/2025)   | `command-r-plus` was an alias for `command-r-plus-04-2024`, so if you use `command-r-plus` in the API, that's the model you're pointing to.                                                                                                          | Text                 | 128k           | 4k                    | [Chat](/reference/chat)|
...
- For tool use, Command R and Command R+ demonstrate improved decision-making around whether or not to use a tool.
